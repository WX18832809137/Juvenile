	...
	Html5video.prototype = {
		...
		isMobile: function() {
			if (mui.os.plus) {
				...
				if (networkTypes[plus.networkinfo.getCurrentType()] != "wifi" && networkTypes[plus.networkinfo.getCurrentType()] !=
					"无网") {
					return false;
				}
			}
		...
		event: function() {
			...
			mui_pay_ico.addEventListener("tap", function() {
				//当暂停或结束
				if (mui_video.paused || mui_video.ended) {
			...
			mui_video.addEventListener("timeupdate", function() {
				...
				mui_block_range.style.backgroundSize = "" + val + "% 100%";
				mui_block_range.style.marginBottom = "20px";
			...
			setInterval(function() {
				...
				if (mui_video.paused || mui_video.ended) {
					...
					if (snum == 3) {
						if (parseInt(mui_video.currentTime) == arr[0] && parseInt(mui_video.currentTime) == arr[1] && parseInt(
								mui_video.currentTime) == arr[2]) {
							rprogress.style.display = "block";
							mui_pay_ico.style.display = "none";
						}
					}
			...
			mui_video.addEventListener("ended", function() {
				if (_this.options.iospay && mui.os.ios) {
					...
					document.getElementById("myBg").style.cssText =
						"position: absolute;left: 0;top: 0;width: 100%;height: 4rem;z-index: 7;display: block;"
			...
			var isfull = function(isstat) {
				if (fullScreenFlag) {
					...
					if (mui.os.ios) {
						//IOS
						_this.getDom.style.cssText =
							"background-color: black;overflow: hidden;width: 100%;height: 4rem;position: relative;";
						if (mui.os.plus) {
					...
					if (mui.os.android) {
						...
						//IOS
						if (mui.os.plus) {
							_this.getDom.style.cssText = "width: 100%;height: 100%;position: absolute;left: 0;top: 0;z-index: 9999;";
